...
(function() {
  var FIXTURE_COOK_ART_313_JSON_URL, FIXTURE_COOK_JSON_URL, JSON_LIST, do_test;
  ...
  JSON_LIST = ['fixture/g1.json', 'fixture/g2.json', 'fixture/g3.json', 'fixture/g4.json', FIXTURE_COOK_JSON_URL, FIXTURE_COOK_ART_313_JSON_URL];
  ...
  seajs.use('graph/net', function(KnowledgeNet) {
    var length, load_next, loaded;
    loaded = [];
    length = JSON_LIST.length;
    load_next = function() {
      var url;
      url = JSON_LIST.shift();
      return jQuery.getJSON(url, function(obj) {
        var art, cook, g1, g2, g3, g4;
        loaded.push(obj);
        if (loaded.length < length) {
          return load_next();
        } else {
          g1 = loaded[0], g2 = loaded[1], g3 = loaded[2], g4 = loaded[3], cook = loaded[4], art = loaded[5];
          return do_test(KnowledgeNet, g1, g2, g3, g4, cook, art);
        }
      });
    };
    return load_next();
  ...
  do_test = function(KnowledgeNet, g1_obj, g2_obj, g3_obj, g4_obj, cook_obj, art_obj) {
    test('JSON Object 检查', function() {
      ok(g1_obj['points'].length === 8);
      return ok(g1_obj['edges'].length === 11);
    });
    (function() {
      var knet, knet2, knet_cook;
      ...
      return test('查找根节点-COOK', function() {
        var roots;
        roots = knet_cook.roots();
        equal(roots.length, 1);
        return ok(knet_cook.is_root('53b23f0f6c696e5c952d0000'));
      });
    })();
    (function() {
      var knet1, knet2, knet3, knet4, knet_art, knet_cook;
      ...
      test('查找多余边-G4', function() {
        var redundant_edges;
        redundant_edges = knet4.get_redundant_edges();
        return deepEqual(redundant_edges.sort(), []);
      });
      test('查找多余边-COOK', function() {
        var redundant_edges;
    ...
    (function() {
      var knet1, knet2, knet3, knet4, knet_cook;
      knet1 = new KnowledgeNet(g1_obj);
      knet2 = new KnowledgeNet(g2_obj);
      knet3 = new KnowledgeNet(g3_obj);
      knet4 = new KnowledgeNet(g4_obj);
      ...
      return test('cook deeps', function() {
        var deeps;
        console.log(knet_cook);
        window.knet = knet_cook;
        deeps = knet_cook.get_deeps();
        return equal(Object.keys(deeps).length, 45);
      });
    })();
    (function() {
    ...
    return (function() {
      ...
      test('对象构建', function() {
        ...
        return equal(knet_art.edges().length, 384);
      });
      test('获取 roots', function() {
        return equal(knet_art.roots().length, 2);
      });
      return test('清理 arts 多余边', function() {
        return equal(knet_art.get_redundant_edges().length, 0);
      });
    })();
  };
